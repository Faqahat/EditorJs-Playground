...
class FontSizeTool {
  ...
  checkState(selection) {
    ...
    const parentNode = text.nodeType === Node.TEXT_NODE ? text.parentNode : text;
  ...
  showFontSizeOptions(range) {
    ...
    this.fontSizes.forEach(fontSizeOption => {
      ...
      option.textContent = fontSizeOption.label + " (" + fontSizeOption.size + ")";
      option.style.cssText = `
        padding: 10px 16px;
        cursor: pointer;
        font-size: ${fontSizeOption.size};
